...
TEST_CASE("deserialize any_value", "[instrument][opentelemetry][protocol][deserialize]")
{
    struct test_case
    {
        const char*      name;
  ...
  "bytesValue": "Zm9v"
            ...
            .expect = {.value = "foo"_bytes},
        });

    SECTION(v.name)
    {
        auto parsed = json::parse(v.input);

        any_value actual;
        parsed.get_to(actual);
        CHECK(actual == v.expect);
    }
}
